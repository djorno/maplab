...
bool OctoTreeNode::recut(VoxHess* vox_opt, resources::PointCloud* points_G) {
  ...
  if (judgeEigen()) {
    ...
    vox_opt->pushVoxel(&index, &sig_orig, &fix_point);
...
void VoxHess::evaluateVoxHess(const vi_map::VIMap* map) {
  ...
  // iterate through the vertices and add the timestamps + ids to the map
  ...
  depth_integration::IntegrationFunctionPointCloudMaplabWithExtras
      integration_function =
          [this, &map, &poses_G_S, &time_last_kf, &last_mission_id,
           &pointclouds, &timestamp_to_vertex](
            ...
            // find the vertex id who's timestamp matches the
  ...
  const int win_size = poses_G_S.size();

  SurfaceMap surface_map;
  for (size_t i = 0; i < win_size; ++i) {
    cutVoxel(surface_map, pointclouds[i], poses_G_S[i], i, win_size);
  }
  ...
  LOG(INFO) << "VoxHess Generated";
}

VoxHessAtom::VoxHessAtom(const VoxHess& voxhess, size_t& feature_index)
  ...
  original_planes.reserve(sig_origin.size());
}

double VoxHessAtom::evaluateResidual(
...
void VoxHessAtom::generateOriginalPlanes() {
  for (size_t sig_i = 0; sig_i < sig_origin.size(); ++sig_i) {
    PointCluster sig_mutable;
    sig_mutable += sig_origin[sig_i];
    CHECK(sig_mutable.N > 0);
...
void VoxHessAtom::evaluatePlane(
  ...
  for (size_t sig_i = 0; sig_i < sig_origin.size(); ++sig_i) {
    ...
    sig_mutable += sig_origin[sig_i].transform(T_M_S);
  }
  Eigen::Vector3d vBar = sig_mutable.v / sig_mutable.N;
  Eigen::Matrix3d cmt = sig_mutable.P / sig_mutable.N - vBar * vBar.transpose();

...
bool BALMErrorTerm::Evaluate(
  ...
  if (evaluation_callback_->preparedForEvaluation() == false) {
  ...
  const std::vector<BALMPlane>& feature_planes_i =
      evaluation_callback_->getFeaturePlanes(feature_index_);
  ...
  for (size_t i = 0; i < feature_index_.size(); i++) {
    ...
    const double sign =
        (feature_planes_i[i].n.dot(R_M_S * original_planes_ij_[i].n) < 0) ? -1.0
                                                                          : 1.0;
    const Eigen::Vector3d& uk_i = sign * feature_planes_i[i].n;
    const Eigen::Vector3d& uk0_ij = original_planes_ij_[i].n;
    const Eigen::Vector3d& pBar_i = feature_planes_i[i].p;
    const Eigen::Vector3d& pBar0_ij = original_planes_ij_[i].p;

    double sigmainv = original_planes_ij_[i].sigmainv;
    // check normals
    ...
    ////////////////////////////// RESIDUALS ////////////////////////////////
    // r_q = (kScalingFactor * N_ij * sigmainv) * common::skew(uk_i) * R_M_S*
    // uk0_ij* acos(uk_i.dot(R_M_S * uk0_ij));
    Eigen::Vector3d r_q =
        (sigmainv != 0.0)
            ? ((kScalingFactor * N_ij * sigmainv) * common::skew(uk_i) * R_M_S *
               uk0_ij * acos(uk_i.dot(R_M_S * uk0_ij)))
                  .eval()
            : Eigen::Vector3d::Zero();  // Set r_q to zero if sigmainv is zero

    Eigen::Vector3d r_p = (kScalingFactor * N_ij) * uk_i * uk_i.transpose() *
  ...
  if (jacobians) {
    ...
    for (size_t i = 0; i < feature_index_.size(); i++) {
      ...
      CHECK(feat_num < evaluation_callback_->getNumFeatures());
      CHECK(
          sig_i <
          evaluation_callback_->getClusterInSensorFrame(feat_num).size());
      const double N_ij = evaluation_callback_->get_N_ij(feat_num, sig_i);
      const double N_i = evaluation_callback_->get_N_i(feat_num);

      const double sign =
          (feature_planes_i[i].n.dot(R_M_S * original_planes_ij_[i].n) < 0)
              ? -1.0
              : 1.0;
      const Eigen::Vector3d& uk_i = sign * feature_planes_i[i].n;
      const Eigen::Vector3d& uk0_ij = original_planes_ij_[i].n;
      ...
      Eigen::Matrix<double, 3, 3> J_r_p_wrt_q_M_S =
          (kScalingFactor * N_ij) * -uk_i * uk_i.transpose() * R_M_S *
          common::skew(pBar0_ij);
      Eigen::Matrix<double, 3, 3> J_r_p_wrt_p_M_S =
          (kScalingFactor * N_ij) * uk_i * uk_i.transpose();
      ////////// computation of the Jacobian of r_q wrt T_M_S
      // Visually;       -(kScalingFactor * N_ij * sigmainv) *
      // common::skew(uk_i) * R_M_S* common::skew(uk0_ij) * acos(uk_i.dot(R_M_S
      // * uk0_ij)) + (kScalingFactor * N_ij * sigmainv) * common::skew(uk_i) *
      // R_M_S* uk0_ij* uk_i.transpose() * R_M_S* common::skew(uk0_ij) /
      // sqrt(1.0 + 1e-6 - pow(uk_i.dot(R_M_S * uk0_ij), 2));
      Eigen::Matrix<double, 3, 3> J_r_q_wrt_q_M_S =
          (sigmainv != 0.0)
              ? ((kScalingFactor * N_ij * sigmainv) *
    ...
    if (jacobians[kIdxPose]) {
      ...
      // Compute the parametrization jacobian.
      // NOTE(gtonetti): Since ceres expects the jacobian wrt the SE(3)
      // parametrization given and we have the jacobian wrt the SE(3) tangent
      // space, we must apply the inverse of the parametrization jacobian to our

...
bool BALMErrorTerm::Evaluate(
  ...
  // Jacobian of the residual w.r.t. the Lidar pose without retraction
  Eigen::Matrix<double, balmblocks::kResidualSize, balmblocks::kPoseSize> J_res_wrt_pose;

  Eigen::Matrix<double, balmblocks::kResidualSize, 3> J_res_wrt_p_G_S;
  Eigen::Matrix<double, balmblocks::kResidualSize, 3> J_res_wrt_q_G_S;

  if (jacobians) {
    // Jacobian w.r.t. the global pose expressed in the global frame.
    if (jacobians[kIdxPose]) {
      Eigen::Map<Eigen::Matrix<double, balmblocks::kResidualSize, balmblocks::kPoseSize, Eigen::RowMajor>> J(jacobians[kIdxPose]);
      J = J_res_wrt_pose;
    }
  }

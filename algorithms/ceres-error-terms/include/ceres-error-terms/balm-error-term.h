 ...
 public:
  BALMEvaluationCallback(
    ...
    for (size_t i = 0; i < num_features_; i++) {
      ...
      voxhess_atoms_[i].generateOriginalPlanes();
  ...
  double get_num_obs_for_feature(const size_t feat_ind) const {
  ...
  std::vector<Eigen::Vector3d> get_vBar_i(
  ...
  std::vector<BALMPlane> get_feature_planes(
      const std::vector<std::pair<size_t, size_t>> feature_index) const {
